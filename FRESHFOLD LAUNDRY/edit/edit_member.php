...
$query = mysqli_query($connection, "SELECT * FROM tb_member WHERE id='$id'");
...
<body>
    <div id="all">
        ...
        <form action="edit/proses_edit_member.php" method="post" id="form">
            <input type="text" hidden id="" name="id" value="<?=$row['id']?>">

            <label for="nama">Nama </label>
            <input type="text" id="nama" name="nama_member" value="<?=$row['nama']?>">

            <label for="alamat">Alamat</label>
            <input type="text" id="alamat" name="alamat" value="<?=$row['alamat']?>">

            <label for="">Jenis Kelamin</label>
            <select name="jenis_kelamin" id="" <?=$row['jenis_kelamin']?> required>
